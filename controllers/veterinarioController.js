...
const olvidePassword = async (req, res)=>{
    ...
    try {
        ...
        res.json({msg:"Hemos enviado las instrucciones a tu casilla de correo"})
    } catch (error) {
        console.log(error)
    }
...
const actualizarPerfil = async (req,res) => {
   ...
   try {
       ...
       await veterinario.save();
       res.json({msg:'veterinario actualizado correctamente'})
   } catch (error) {
    console.log(error)
   }
}
...
if(await veterinario.comprobarPassword(pwd_actual)){
   veterinario.password = pwd_nuevo;
    ...
    const error = new Error("La password actual  no es correcto");
    return res.status(400).json({msg: error.message});
}
}
